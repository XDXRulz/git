 ...
 **/
		datePicker: function(s) {
			...
			return this.each(
				function()
				{
					...
					if (!this._dpId) {
						this._dpId = $.guid++;//zhivko: $.event.guid++;
					...
					if (!alreadyExists && s.createButton) {
						controller.button = $('<a href="#" class="dp-choose-date" title="'
                                + $.dpText.TEXT_CHOOSE_DATE + '">' + $.dpText.TEXT_CHOOSE_DATE + '</a>')
                            .bind('click', function() {
                                $this.dpDisplay(this);
                                this.blur();
                                return false;
					...
					if (!alreadyExists && $this.is(':text')) {
						...
						if (s.clickInput) {
							$this.bind('click', function() { $this.dpDisplay(); });
						}
						var d = Date.fromString(this.value);
						if (this.value != '' && d) {
							controller.setSelected(d, true, true);
         ...
         * @desc Prevents this date picker from displaying and adds a class of dp-disabled to it
         *      (and it's associated button if it has one) for styling purposes. If the matched
         *      element is an input field then it will also set the disabled attribute to stop
         *      people directly editing the field.
         **/
 ...
 **/
	...
	$.extend(DatePicker.prototype, {
			...
			display : function(eleAlignTo)
			{
                ...
                if (fixedPopup()) {
                    $('div#dp-popup').css('position', 'fixed');
                }
